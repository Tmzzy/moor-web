...
use crate::core::db::{settings_repo, Database};
...
mod tests {
    ...
    fn migrates_settings_json_once_and_keeps_database_as_source_of_truth() {
        ...
        fs::write(
            ...
            r#"{
              ...
              "advanced": { "mcpRequestTimeoutMs": 45000 }
            }"#,
        )
        ...
        let updated = update_settings(
            &db,
            serde_json::json!({
                "general": { "autoStartServersOnLaunch": false },
                "advanced": { "mcpRequestTimeoutMs": 60000 }
            }),
        )
        .expect("settings update should succeed");

        assert!(migrated.general.auto_start_servers_on_launch);
        assert_eq!(migrated.advanced.mcp_request_timeout_ms, 45_000);
        assert!(!updated.general.auto_start_servers_on_launch);
        assert_eq!(updated.advanced.mcp_request_timeout_ms, 60_000);
        assert!(
            read_settings_file(&data_dir)
                .general
                .auto_start_servers_on_launch
        );
        assert_eq!(
            read_settings_file(&data_dir)
                .advanced
    ...
    #[test]
    fn ignores_removed_desktop_settings() {
        let data_dir = temp_data_dir("removed-settings");
        let db = test_db(&data_dir);
        ...
        let settings = update_settings(
            &db,
            serde_json::json!({
                "general": { "autoStartOnLogin": true },
                "advanced": { "sidecarPort": 80 }
            }),
        )
        .expect("removed settings should be ignored for backward compatibility");
        assert_eq!(settings, default_settings());
        let _ = fs::remove_dir_all(data_dir);
    }
